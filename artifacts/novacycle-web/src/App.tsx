...
import { Activity, Server, Clock, Download, ExternalLink, Terminal, AlertTriangle, CheckCircle2, RotateCcw, KeyRound, X, TrendingUp, TrendingDown, Minus, Gauge } from 'lucide-react';
...
function confidenceZone(pct: number): { text: string; bar: string; border: string; bg: string } {
  if (pct >= 65) {
    return { text: 'text-emerald-400', bar: 'bg-emerald-400', border: 'border-emerald-400/20', bg: 'bg-emerald-400/10' };
  }
  if (pct >= 31) {
    return { text: 'text-amber-400', bar: 'bg-amber-400', border: 'border-amber-400/20', bg: 'bg-amber-400/10' };
  }
  return { text: 'text-red-400', bar: 'bg-red-400', border: 'border-red-400/20', bg: 'bg-red-400/10' };
}
...
function PredictionCard({ name, label }: { name: string; label: string }) {
  ...
  if (noData) {
    return (
      ...
      >
        ...
        <div className="font-mono text-2xl text-muted-foreground" data-testid={`text-confidence-${name}`}>—</div>
